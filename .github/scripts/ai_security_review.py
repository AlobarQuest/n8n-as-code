...
prompt = (
    ...
    "IMPORTANT: I run with telemetry DISABLED (opt-out). The tool ships telemetry "
    "enabled-by-default; I need to know if this release changes the telemetry default, "
    "weakens opt-out, resets/migrates the user telemetry config, or adds a new telemetry "
    "path — so I can re-disable it after updating.\n\n"
    ...
    "(new endpoints, new data fields, consent/opt-out removal, default flipped to enabled, "
    "opt-out→opt-in, or config reset/migration logic)\n"
    ...
    "3. Report the telemetry default state explicitly (see Telemetry section below).\n"
    "4. Give a one-line recommendation.\n\n"
    ...
    "## Telemetry\n"
    "[State whether this release changes the telemetry default/opt-out behavior. "
    "One of: NO CHANGE / DEFAULT NOW ENABLED / OPT-OUT WEAKENED / CONFIG RESET-MIGRATION — "
    "with file:line. If anything other than NO CHANGE, remind: re-run `n8nac telemetry disable` "
    "after rebuilding.]\n\n"
